...
parser.add_argument("-m", "--manual", help="run the Manuel model instead of the AI model", 
                    action="store_true")
...
if args.manual:
    with open("manual_flag.txt", "w+") as f: #save the manual flag for the re run of app
        f.write("True")
    ...
    for i in range(opt.demo_len):
        ...
        if not args.localdemo:
            ...
            try:
                ...
                manuelModel.min_temp = int(data['minTemp'])
                manuelModel.max_temp = int(data['maxTemp'])
                manuelModel.max_allowed_temp_change = int(data['rateOfChange'])  #update manuel model
                manuelModel.crit_min_temp = int(data['critMinTemp'])
                manuelModel.crit_max_temp = int(data['critMaxTemp'])
                manuelModel.crit_time = int(data['maxTime'])
